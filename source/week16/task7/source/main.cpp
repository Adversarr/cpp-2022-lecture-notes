...
class ANNCell {
  double x[MAX],
      w[MAX]; // 数组存储神经元的输入节点值Xi，以及此输入节点的权重Wi,
  double bias; // 神经元的偏置值
...
public:
  Sigmoid(double in[], double weight[], int num, double b)
      : ANNCell(in, weight, num, b) {}
  ~Sigmoid() {}
  double fun(double y) {
    //完善代码
    return 1 / (1.0 + exp(-y));
...
public:
  Tanh(double in[], double weight[], int num, double b)
      : ANNCell(in, weight, num, b) {}
  ~Tanh() {}
  double fun(double y) {
    // 完善代码
    // NOTE: return tanh(y);
    double ey = exp(y);
    double e_neg_y = exp(-y);
    return (ey - e_neg_y) / (ey + e_neg_y);
  }
};
