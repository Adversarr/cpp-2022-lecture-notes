...
Array &operator=(const Array &arr); //赋值运算符重载
  ...
  friend ostream &operator<<(ostream &dist, Array<Tp> &ar); //重载插入运算符
};
...
//清空线性表
template <typename T> void Array<T>::InitArray() { Subscript = -1; }

//插入流操作符重载
template<typename T> ostream& operator<<(ostream& dist, Array<T> &ar) {
  for (int i = 0; i <= ar.Subscript; ++i) {
    dist << ar.elements[i];
  }
  return dist;
}
...
double CPolygon::CalPerimeter() {
  //添加代码
  perimeter = VexList[0].Distance(VexList[VexList.GetElemNum() - 1]);
  for (int i = 1; i < VexList.GetElemNum(); ++i) {
    perimeter += VexList[i].Distance(VexList[i - 1]); 
  }
  return perimeter;
}
